...
def preprocess_input(input_df):
    ...
    if ohe_cols_in_df:
        try:
            ...
            # Handle unseen categories by replacing them with a known category before encoding
            for col in ohe_cols_in_df:
                 if col in df_processed.columns:
                     try:
                         ...
                         # Find the first non-NaN category from the encoder's categories
                         first_valid_category = next((str(cat) for cat in encoder_categories if pd.notna(cat)), None)

                         fill_value = first_valid_category if first_valid_category is not None else 'DefaultCategory' # Use a distinct default

                         # Replace any value in the input column that is not in the encoder's categories (excluding NaN)
                         # with the fill_value.
                         ...
                         # Fill original NaNs with the same fill_value
                         df_processed[col] = df_processed[col].fillna(fill_value).astype(str)
                          ...
                          # Fallback fill if column not in encoder or error occurs
                          df_processed[col] = df_processed[col].fillna('DefaultCategory').astype(str)


            # Apply One-Hot Encoding
            # Ensure the columns to be transformed are of object/string type after filling NaNs
