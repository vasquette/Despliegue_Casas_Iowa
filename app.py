...
def get_input_data_from_widgets():
    ...
    input_data['PoolQC'] = st.sidebar.selectbox('PoolQC', options=[np.nan, 'Ex', 'Gd', 'TA', 'Fa'], index=0, key='PoolQC')
    input_data['Fence'] = st.sidebar.selectbox('Fence', options=[np.nan, 'MnPrv', 'GdWo', 'GdPrv', 'MnWw'], index=0, key='Fence')
    input_data['MiscFeature'] = st.sidebar.selectbox('MiscFeature', options=[np.nan, 'Shed', 'Gar2', 'Othr', 'TenC'], index=0, key='MiscFeature')
