...
def preprocess_input(input_df):
    ...
    if ohe_cols_in_df:
        try:
            ...
            # Fill NaN values in categorical columns with the first category from the encoder's classes
            for col in ohe_cols_in_df:
                if col in df_processed.columns:
                    try:
                        ...
                        first_valid_category = next((cat for cat in encoder_categories if pd.notna(cat)), None)

                        fill_value = first_valid_category if first_valid_category is not None else 'UnknownCategory' # Use a placeholder if no valid category

                        # Replace any value not in encoder's categories (excluding NaN which will be filled)
                        # with the fill_value before filling NaNs.
                        # This is a more robust way to handle unseen categories if handle_unknown='ignore' is not fully effective
                        ...
                        df_processed[col] = df_processed[col].fillna(fill_value).astype(str) # Fill remaining NaN and ensure string type
                         ...
                         st.warning(f"Column '{col}' not found in onehot_encoder categories. Skipping fill with first category.")
