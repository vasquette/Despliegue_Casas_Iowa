...
# --- Define the exact list of categorical columns the OneHotEncoder was trained on ---
# This list needs to be manually defined based on the training script
# Assuming the order was consistent during training
OHE_TRAINED_COLS = ['MSZoning', 'LotShape', 'LandContour', 'LotConfig', 'Neighborhood', 'Condition1', 'BldgType', 'HouseStyle', 'Exterior1st', 'Foundation', 'BsmtExposure', 'BsmtFinType1', 'BsmtFinType2', 'CentralAir', 'Electrical', 'GarageType', 'GarageFinish', 'PavedDrive', 'SaleType', 'SaleCondition']
...
def preprocess_input(input_df):
    ...
    # Explicitly use the predefined list of columns the encoder was trained on
    ohe_cols_to_transform = [col for col in OHE_TRAINED_COLS if col in df_processed.columns]

    # Check if the number of columns in the input DataFrame matches the number expected by the encoder
    if len(ohe_cols_to_transform) != len(OHE_TRAINED_COLS):
        missing_cols = set(OHE_TRAINED_COLS) - set(ohe_cols_to_transform)
        st.error(f"Preprocessing error: Input DataFrame is missing expected categorical columns for One-Hot Encoding: {missing_cols}")
        st.write(f"Expected OHE columns: {OHE_TRAINED_COLS}")
        st.write(f"Available OHE columns in input DF: {ohe_cols_to_transform}")
        ...
        try:
            ...
            for col in ohe_cols_to_transform:
                 if col in df_processed_ohe_subset.columns:
                     ...
                     # Replace the string 'nan' with a placeholder if 'nan' is not an expected category
                     # This is a fallback if handle_unknown='ignore' isn't sufficient or available
                     col_index_in_encoder = OHE_TRAINED_COLS.index(col)
                     encoder_categories_for_col = [str(cat) for cat in onehot_encoder.categories_[col_index_in_encoder] if pd.notna(cat)]
                     if 'nan' not in encoder_categories_for_col and 'nan' in df_processed_ohe_subset[col].unique():
                         df_processed_ohe_subset[col] = df_processed_ohe_subset[col].replace('nan', 'DefaultCategory') # Use a consistent placeholder
    ...
    if EXPECTED_FEATURES_AFTER_PREPROCESSING is not None and len(EXPECTED_FEATURES_AFTER_PREPROCESSING) > 0:
        ...
        for col in df_aligned.columns:
            if col in df_processed.columns:
                df_aligned[col] = df_aligned[col].astype(df_processed[col].dtype) # Preserve original dtype before filling
