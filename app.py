...
# --- Define the exact list of original categorical columns expected by the OneHotEncoder ---
# This list needs to be manually defined based on the training script
# Exclude columns that are expected to be handled differently (e.g., Label Encoded or dropped)
OHE_EXPECTED_COLS_TRAINED = [
    'MSZoning', 'LotShape', 'LandContour', 'LotConfig', 'Neighborhood',
    'Condition1', 'BldgType', 'HouseStyle', 'Exterior1st', 'Foundation',
    'BsmtExposure', 'BsmtFinType1', 'BsmtFinType2', 'CentralAir', 'Electrical',
    'GarageType', 'PavedDrive', 'SaleType', 'SaleCondition' # Excluded GarageFinish based on analysis
    ]
...
def get_input_data_from_widgets():
    ...
    widget_key = 'MSZoning'
    widget_keys.add(widget_key)
    # Remove 'C (all)' as an option from the MSZoning selectbox
    input_data[widget_key] = st.sidebar.selectbox('MSZoning', options=['RL', 'RM', 'FV', 'RH'], index=0, key=widget_key)
...
def preprocess_input(input_df):
    ...
    # Use the predefined OHE_EXPECTED_COLS_TRAINED list to identify columns that should be objects/categories
    cols_to_convert_to_category = [col for col in OHE_EXPECTED_COLS_TRAINED if col in df_processed.columns]

    for col in cols_to_convert_to_category:
         if col in df_processed.columns:
            # Ensure NaNs are not converted to string 'nan' before converting to category if NaN is a valid category
            # For simplicity, let's fill NaNs with a placeholder before converting to category,
            # unless 'nan' was a known category in the encoder (less likely for these columns).
            # A safer approach is to ensure string type for OHE input later.
            # df_processed[col] = df_processed[col].astype('category')
            pass # Skipping direct category conversion here and will handle string conversion before OHE
    ...
    label_encoders = {
        ...
        'GarageFinish': garagefinish_label_encoder # Included GarageFinish for Label Encoding
    }
        ...
        if col in df_processed.columns: # Check if column exists before processing
             ...
             try:
                 ...
                 if not pd.api.types.is_object_dtype(df_processed[col]) and not pd.api.types.is_categorical_dtype(df_processed[col]):
                     df_processed[col] = df_processed[col].astype(str) # Convert to string if not object/category
    ...
    # 3. Apply One-Hot Encoding (ensure columns exist and handle missing/extra/unseen categories)
    # Use the predefined list of columns the encoder was trained on and are expected to be OHE
    ohe_cols_to_transform = [col for col in OHE_EXPECTED_COLS_TRAINED if col in df_processed.columns]

    # Check if the number of columns in the input DataFrame slice matches the number expected by the encoder's categories
    # This is a more direct check against the encoder's structure
    if models_loaded and hasattr(onehot_encoder, 'categories_'):
        num_encoder_categories_cols = len(onehot_encoder.categories_)
        if len(ohe_cols_to_transform) != num_encoder_categories_cols:
             st.error(f"Preprocessing error: Number of columns ({len(ohe_cols_to_transform)}) identified for OHE does not match the number of categories sets in the encoder ({num_encoder_categories_cols}). Check OHE_EXPECTED_COLS_TRAINED.")
             st.write(f"Columns to transform: {ohe_cols_to_transform}")
             st.write(f"Encoder categories structure: {onehot_encoder.categories_}")
             return None # Indicate failure if column count doesn't match encoder's categories structure

    # Ensure the columns are in the correct order for transformation
    # Since OHE_EXPECTED_COLS_TRAINED is manually defined in the expected order,
    # we just need to ensure the subset from df_processed is taken in that order.
    ...
    if models_loaded and hasattr(onehot_encoder, 'categories_'):
        ...
        for i, cats in enumerate(onehot_encoder.categories_):
            # Attempt to match category index to column name based on OHE_EXPECTED_COLS_TRAINED
            col_name = OHE_EXPECTED_COLS_TRAINED[i] if i < len(OHE_EXPECTED_COLS_TRAINED) else f"Unknown_col_{i}"
        ...
        try:
            ...
            for col in ohe_cols_to_transform:
                 if col in df_processed_ohe_subset.columns:
                     # Replace NaN with a placeholder string *before* converting to string
                     # This ensures 'nan' isn't created from actual NaNs if 'nan' isn't a true category
                     if df_processed_ohe_subset[col].isnull().any():
                         df_processed_ohe_subset[col] = df_processed_ohe_subset[col].fillna('Missing') # Use 'Missing' as a placeholder

                     df_processed_ohe_subset[col] = df_processed_ohe_subset[col].astype(str)
    ...
    all_numeric_cols = numerical_cols_to_scale + label_encode_cols # Combine lists
    ...
    if EXPECTED_FEATURES_AFTER_PREPROCESSING is not None and len(EXPECTED_FEATURES_AFTER_PREPROCESSING) > 0:
        ...
        for col in df_aligned.columns:
            if col in df_processed.columns:
                 # Ensure the data type is compatible before assignment, especially if it's already numeric
        ...
        st.write("Expected features for PCA (shape):", EXPECTED_FEATURES_AFTER_PREPROCESSING.shape)

        # Final check: ensure number of columns matches expected features for PCA
        if df_aligned.shape[1] != len(EXPECTED_FEATURES_AFTER_PREPROCESSING):
            st.error(f"Column mismatch before PCA: DataFrame has {df_aligned.shape[1]} columns, but PCA expects {len(EXPECTED_FEATURES_AFTER_PREPROCESSING)}. Cannot apply PCA.")
            # Optionally print column differences for debugging
            # missing_from_aligned = set(EXPECTED_FEATURES_AFTER_PREPROCESSING) - set(df_aligned.columns)
            # extra_in_aligned = set(df_aligned.columns) - set(EXPECTED_FEATURES_AFTER_PREPROCESSING)
            # st.write(f"Missing from aligned DF: {missing_from_aligned}")
            # st.write(f"Extra in aligned DF: {extra_in_aligned}")
            return None
