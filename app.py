...
import numpy as np
import scipy.sparse
import base64 # To offer download link
    ...
    for col in cols_for_widgets:
        ...
        if col in categorical_options:
            ...
            try:
                 ...
                 # Handle the case where default_value is np.nan and we need its index
                 if pd.isna(default_value):
                     if np.nan in options:
                          default_index = options.index(np.nan)
                     else:
                          default_index = 0 # Fallback if np.nan not in options
                 else:
...
# Add a file uploader option
st.sidebar.header('Upload Data File (CSV)')
...
if uploaded_file is not None:
    ...
    try:
        ...
        st.write("Using uploaded file for predictions.")
        # Display the first few rows of the uploaded data for confirmation
        st.subheader("Uploaded Data Preview:")
        st.dataframe(uploaded_df.head())
        input_df = uploaded_df # Use the uploaded data as input
    except Exception as e:
        st.error(f"Error reading uploaded file: {e}")
        st.write("Please use the sidebar inputs instead.")
        input_df = get_input_data_from_widgets() # Fallback to manual input if file upload fails
else:
    st.write("Or enter house characteristics manually in the sidebar.")
    # Use the manual input widgets if no file is uploaded or upload failed
        ...
        if 'loaded_model' in locals() and processed_df is not None:
            ...
            if hasattr(loaded_model, 'n_features_in_'):
                if processed_df.shape[1] != loaded_model.n_features_in_:
                    ...
                    st.subheader("Predictions:") # Changed subheader for multiple predictions

                    if len(predictions) > 1:
                        # Display predictions for uploaded file as a DataFrame
                        predictions_df = pd.DataFrame({'Predicted SalePrice': predictions})
                        st.dataframe(predictions_df)

                        # Add a download link for the predictions
                        csv = predictions_df.to_csv(index=False)
                        b64 = base64.b64encode(csv.encode()).decode()  # b64 encode CSV
                        linko= f'<a href="data:file/csv;base64,{b64}" download="predictions.csv">Download Predictions CSV</a>'
                        st.markdown(linko, unsafe_allow_html=True)

                    else:
                        # Display single prediction for manual input
