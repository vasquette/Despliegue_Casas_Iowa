...
def get_input_data_from_widgets():
    ...
    input_data['MSSubClass'] = st.sidebar.selectbox('MSSubClass', options=[20, 30, 40, 45, 50, 60, 70, 75, 80, 85, 90, 120, 150, 160, 180, 190], index=0)
    input_data['LotFrontage'] = st.sidebar.number_input('LotFrontage', min_value=0.0, value=70.0, step=1.0)
    input_data['LotArea'] = st.sidebar.number_input('LotArea', min_value=0, value=10500, step=100)
    input_data['OverallQual'] = st.sidebar.slider('OverallQual', 1, 10, 5)
    input_data['OverallCond'] = st.sidebar.slider('OverallCond', 1, 9, 5)
    input_data['YearBuilt'] = st.sidebar.number_input('YearBuilt', min_value=1800, value=1980, step=1)
    input_data['YearRemodAdd'] = st.sidebar.number_input('YearRemodAdd', min_value=1800, value=1980, step=1)
    input_data['MasVnrArea'] = st.sidebar.number_input('MasVnrArea', min_value=0.0, value=0.0, step=10.0)
    input_data['BsmtFinSF1'] = st.sidebar.number_input('BsmtFinSF1', min_value=0, value=500, step=10)
    input_data['BsmtFinSF2'] = st.sidebar.number_input('BsmtFinSF2', min_value=0, value=0, step=10)
    input_data['BsmtUnfSF'] = st.sidebar.number_input('BsmtUnfSF', min_value=0, value=500, step=10)
    input_data['TotalBsmtSF'] = st.sidebar.number_input('TotalBsmtSF', min_value=0, value=1000, step=10)
    input_data['1stFlrSF'] = st.sidebar.number_input('1stFlrSF', min_value=0, value=1000, step=10)
    input_data['2ndFlrSF'] = st.sidebar.number_input('2ndFlrSF', min_value=0, value=0, step=10)
    input_data['LowQualFinSF'] = st.sidebar.number_input('LowQualFinSF', min_value=0, value=0, step=10)
    input_data['GrLivArea'] = st.sidebar.number_input('GrLivArea', min_value=0, value=1000, step=10)
    input_data['BsmtFullBath'] = st.sidebar.number_input('BsmtFullBath', min_value=0, value=0, step=1)
    input_data['BsmtHalfBath'] = st.sidebar.number_input('BsmtHalfBath', min_value=0, value=0, step=1)
    input_data['FullBath'] = st.sidebar.number_input('FullBath', min_value=0, value=1, step=1)
    input_data['HalfBath'] = st.sidebar.number_input('HalfBath', min_value=0, value=0, step=1)
    input_data['BedroomAbvGr'] = st.sidebar.number_input('BedroomAbvGr', min_value=0, value=3, step=1)
    input_data['KitchenAbvGr'] = st.sidebar.number_input('KitchenAbvGr', min_value=0, value=1, step=1)
    input_data['TotRmsAbvGrd'] = st.sidebar.number_input('TotRmsAbvGrd', min_value=0, value=5, step=1)
    input_data['Fireplaces'] = st.sidebar.number_input('Fireplaces', min_value=0, value=0, step=1)
    input_data['GarageYrBlt'] = st.sidebar.number_input('GarageYrBlt', min_value=1800.0, value=1980.0, step=1.0)
    input_data['GarageCars'] = st.sidebar.number_input('GarageCars', min_value=0, value=1, step=1)
    input_data['GarageArea'] = st.sidebar.number_input('GarageArea', min_value=0, value=300, step=10)
    input_data['WoodDeckSF'] = st.sidebar.number_input('WoodDeckSF', min_value=0, value=0, step=10)
    input_data['OpenPorchSF'] = st.sidebar.number_input('OpenPorchSF', min_value=0, value=0, step=10)
    input_data['EnclosedPorch'] = st.sidebar.number_input('EnclosedPorch', min_value=0, value=0, step=10)
    input_data['3SsnPorch'] = st.sidebar.number_input('3SsnPorch', min_value=0, value=0, step=10)
    input_data['ScreenPorch'] = st.sidebar.number_input('ScreenPorch', min_value=0, value=0, step=10)
    input_data['PoolArea'] = st.sidebar.number_input('PoolArea', min_value=0, value=0, step=10)
    input_data['MiscVal'] = st.sidebar.number_input('MiscVal', min_value=0, value=0, step=10)
    input_data['MoSold'] = st.sidebar.slider('MoSold', 1, 12, 6)
    input_data['YrSold'] = st.sidebar.slider('YrSold', 2006, 2010, 2008)


    # Widgets para variables categóricas (ejemplos con opciones comunes)
    input_data['MSZoning'] = st.sidebar.selectbox('MSZoning', options=['RL', 'RM', 'C (all)', 'FV', 'RH'], index=0)
    input_data['Street'] = st.sidebar.selectbox('Street', options=['Pave', 'Grvl'], index=0)
    input_data['Alley'] = st.sidebar.selectbox('Alley', options=[np.nan, 'Grvl', 'Pave'], index=0)
    input_data['LotShape'] = st.sidebar.selectbox('LotShape', options=['Reg', 'IR1', 'IR2', 'IR3'], index=0)
    input_data['LandContour'] = st.sidebar.selectbox('LandContour', options=['Lvl', 'Bnk', 'HLS', 'Low'], index=0)
    input_data['Utilities'] = st.sidebar.selectbox('Utilities', options=['AllPub', 'NoSeWa'], index=0)
    input_data['LotConfig'] = st.sidebar.selectbox('LotConfig', options=['Inside', 'Frontage', 'Corner', 'CulDSac', 'FR2', 'FR3'], index=0)
    input_data['LandSlope'] = st.sidebar.selectbox('LandSlope', options=['Gtl', 'Mod', 'Sev'], index=0)
    input_data['Neighborhood'] = st.sidebar.selectbox('Neighborhood', options=['NAmes', 'CollgCr', 'OldTown', 'Edwards', 'Somerst', 'Gilbert', 'NridgHt', 'Sawyer', 'NWAmes', 'SawyerW', 'BrkSide', 'Crawfor', 'Mitchel', 'NoRidge', 'Timber', 'IDOTC', 'ClearCr', 'StoneBr', 'SWISU', 'Blmngtn', 'MeadowV', 'BrDale', 'Veenker', 'NPkVill', 'Blueste'], index=0)
    input_data['Condition1'] = st.sidebar.selectbox('Condition1', options=['Norm', 'Feedr', 'Artery', 'RRAn', 'PosN', 'RRAe', 'PosA', 'RRNn', 'RRNe'], index=0)
    input_data['Condition2'] = st.sidebar.selectbox('Condition2', options=['Norm', 'Artery', 'Feedr', 'RRNn', 'RRAn', 'RRAe', 'PosN', 'PosA'], index=0)
    input_data['BldgType'] = st.sidebar.selectbox('BldgType', options=['1Fam', '2FmCon', 'Duplex', 'TwnhsE', 'Twnhs'], index=0)
    input_data['HouseStyle'] = st.sidebar.selectbox('HouseStyle', options=['1Story', '2Story', '1.5Fin', '1.5Unf', 'SFoyer', 'SLvl', '2.5Unf', '2.5Fin'], index=0)
    input_data['RoofStyle'] = st.sidebar.selectbox('RoofStyle', options=['Gable', 'Hip', 'Flat', 'Gambrel', 'Mansard', 'Shed'], index=0)
    input_data['RoofMatl'] = st.sidebar.selectbox('RoofMatl', options=['CompShg', 'WdShngl', 'Metal', 'WdShake', 'Membran', 'Roll', 'ClyTile'], index=0)
    input_data['Exterior1st'] = st.sidebar.selectbox('Exterior1st', options=['VinylSd', 'MetalSd', 'Wd Sdng', 'HdBoard', 'BrkFace', 'WdShing', 'CemntBd', 'Plywood', 'AsbShng', 'Stucco', 'CBlock', 'BrkComm', 'AsphShn', 'VinylSd', 'ImStucc', 'Stone', 'Other'], index=0)
    input_data['Exterior2nd'] = st.sidebar.selectbox('Exterior2nd', options=['VinylSd', 'MetalSd', 'Wd Shng', 'HdBoard', 'BrkFace', 'Wd Sdng', 'CemntBd', 'Plywood', 'AsbShng', 'Stucco', 'CBlock', 'BrkComm', 'AsphShn', 'VinylSd', 'ImStucc', 'Stone', 'Other'], index=0)
    input_data['MasVnrType'] = st.sidebar.selectbox('MasVnrType', options=['None', 'BrkFace', 'Stone', 'BrkCmn', np.nan], index=0)
    input_data['Foundation'] = st.sidebar.selectbox('Foundation', options=['PConc', 'CBlock', 'BrkTil', 'Wood', 'Slab', 'Stone'], index=0)
    input_data['BsmtQual'] = st.sidebar.selectbox('BsmtQual', options=['Gd', 'TA', 'Ex', 'Fa', np.nan], index=1) # Usar index=1 para 'TA' como defecto
    input_data['BsmtCond'] = st.sidebar.selectbox('BsmtCond', options=['TA', 'Gd', 'Fa', 'Po', np.nan], index=0) # Usar index=0 para 'TA' como defecto
    input_data['BsmtExposure'] = st.sidebar.selectbox('BsmtExposure', options=['No', 'Gd', 'Mn', 'Av', np.nan], index=0)
    input_data['BsmtFinType1'] = st.sidebar.selectbox('BsmtFinType1', options=['GLQ', 'ALQ', 'Unf', 'Rec', 'BLQ', 'LwQ', np.nan], index=1) # Usar index=1 para 'ALQ' como defecto
    input_data['BsmtFinType2'] = st.sidebar.selectbox('BsmtFinType2', options=['Unf', 'Rec', 'LwQ', 'GLQ', 'ALQ', 'BLQ', np.nan], index=0) # Usar index=0 para 'Unf' como defecto
    input_data['Heating'] = st.sidebar.selectbox('Heating', options=['GasA', 'GasW', 'Grav', 'Wall', 'OthW', 'Floor'], index=0)
    input_data['HeatingQC'] = st.sidebar.selectbox('HeatingQC', options=['Ex', 'Gd', 'TA', 'Fa', 'Po'], index=0) # Usar index=0 para 'Ex' como defecto
    input_data['CentralAir'] = st.sidebar.selectbox('CentralAir', options=['Y', 'N'], index=0)
    input_data['Electrical'] = st.sidebar.selectbox('Electrical', options=['SBrkr', 'FuseA', 'FuseF', 'FuseP', 'Mix', np.nan], index=0)
    input_data['KitchenQual'] = st.sidebar.selectbox('KitchenQual', options=['Gd', 'TA', 'Ex', 'Fa'], index=1) # Usar index=1 para 'TA' como defecto
    input_data['Functional'] = st.sidebar.selectbox('Functional', options=['Typ', 'Min1', 'Maj1', 'Min2', 'Mod', 'Maj2', 'Sev'], index=0) # Usar index=0 para 'Typ' como defecto
    input_data['FireplaceQu'] = st.sidebar.selectbox('FireplaceQu', options=[np.nan, 'Gd', 'TA', 'Fa', 'Ex', 'Po'], index=0)
    input_data['GarageType'] = st.sidebar.selectbox('GarageType', options=['Attchd', 'Detchd', 'BuiltIn', 'CarPort', np.nan, 'Basment', '2Types'], index=0)
    input_data['GarageYrBlt'] = st.sidebar.number_input('GarageYrBlt', min_value=1800.0, value=1980.0, step=1.0)
    input_data['GarageFinish'] = st.sidebar.selectbox('GarageFinish', options=['Unf', 'RFn', 'Fin', np.nan], index=0) # Usar index=0 para 'Unf' como defecto
    input_data['GarageQual'] = st.sidebar.selectbox('GarageQual', options=[np.nan, 'TA', 'Fa', 'Gd', 'Ex', 'Po'], index=0)
    input_data['GarageCond'] = st.sidebar.selectbox('GarageCond', options=[np.nan, 'TA', 'Fa', 'Gd', 'Po', 'Ex'], index=0)
    input_data['PavedDrive'] = st.sidebar.selectbox('PavedDrive', options=['Y', 'N', 'P'], index=0)
    input_data['Fence'] = st.sidebar.selectbox('Fence', options=[np.nan, 'MnPrv', 'GdWo', 'GdPrv', 'MnWw'], index=0)
    input_data['MiscFeature'] = st.sidebar.selectbox('MiscFeature', options=[np.nan, 'Shed', 'Gar2', 'Othr', 'TenC'], index=0)
    input_data['SaleType'] = st.sidebar.selectbox('SaleType', options=['WD', 'New', 'COD', 'ConLD', 'ConLI', 'CWD', 'ConLw', 'Con', 'Oth'], index=0)
    input_data['SaleCondition'] = st.sidebar.selectbox('SaleCondition', options=['Normal', 'Abnorml', 'Partial', 'AdjLand', 'Alloca', 'Family'], index=0)
