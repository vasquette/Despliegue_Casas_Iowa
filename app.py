...
def preprocess_input(input_df):
    ...
    categorical_cols_ohe = ['MSZoning', 'LotShape', 'LandContour', 'LotConfig', 'Neighborhood', 'Condition1', 'BldgType', 'HouseStyle', 'Exterior1st', 'Foundation', 'BsmtExposure', 'BsmtFinType1', 'BsmtFinType2', 'CentralAir', 'Electrical', 'GarageType', 'GarageFinish', 'PavedDrive', 'SaleType', 'SaleCondition']
    ...
    if ohe_cols_in_df:
        try:
            ...
            # Although we are handling unseen categories manually below, this is good practice
            if hasattr(onehot_encoder, 'handle_unknown'):
                 onehot_encoder.handle_unknown = 'ignore'
            else:
                st.warning("OneHotEncoder does not support handle_unknown='ignore'. Relying on manual unseen category handling.")


            # Prepare columns for One-Hot Encoding: ensure they are string type and handle unseen categories
            for col in ohe_cols_in_df:
                 if col in df_processed.columns:
                     try:
                         # Get valid categories from the encoder for this column
                         ...
                         # Find a suitable fill value from the encoder's categories
                         first_valid_category = next((cat for cat in valid_encoder_categories_list if cat != 'DefaultCategory'), None) # Avoid using previous fallback
                         fill_value = first_valid_category if first_valid_category is not None else 'DefaultCategory' # Use a distinct default

                         # Convert column to string type to avoid issues with mixed types and NaNs
                         df_processed[col] = df_processed[col].astype(str)

                         # Replace any value that is not in the encoder's known categories (and not the string 'nan')
                         # with the determined fill_value. Explicitly handle 'nan' string if it exists.
                         df_processed[col] = df_processed[col].apply(lambda x: fill_value if (x != 'nan' and x not in valid_encoder_categories_list) or (x == 'nan' and 'nan' not in valid_encoder_categories_list) else x)

                         # If 'nan' was a category in the encoder, ensure original NaNs are filled with it, otherwise use fill_value
                         if 'nan' in valid_encoder_categories_list:
                            df_processed[col] = df_processed[col].replace('nan', 'nan').fillna('nan') # Replace string 'nan' with actual NaN then fill
                         else:
                            # Replace string 'nan' with actual NaN before filling with fill_value
                            df_processed[col] = df_processed[col].replace('nan', np.nan).fillna(fill_value)


                     except ValueError:
                          st.warning(f"Column '{col}' not found in onehot_encoder categories or error during category handling. Skipping specific handling.")
                          # Fallback: convert to string and fill any remaining 'nan'
                          df_processed[col] = df_processed[col].astype(str)
                          if 'nan' in df_processed[col].unique():
                               df_processed[col] = df_processed[col].replace('nan', 'DefaultCategory')


            # Apply One-Hot Encoding
            # Ensure the columns to be transformed are of object/string type
    ...
    if EXPECTED_FEATURES_AFTER_PREPROCESSING is not None and len(EXPECTED_FEATURES_AFTER_PROCESSING) > 0:
        ...
        for col in df_aligned.columns:
            if col in df_processed.columns:
                df_aligned[col] = df_processed[col].astype(df_processed[col].dtype) # Preserve original dtype before filling
